...
import { computeSaleState } from './sale.js';
...
describe('computeSaleState boundaries', () => {
  it('60s before start -> upcoming', () => {
    expect(computeSaleState(START, END, START - MIN).status).toBe('upcoming');
  });
  ...
  it('mid-window -> active', () => {
    expect(computeSaleState(START, END, START + 5 * MIN).status).toBe('active');
  });

  it('exact end -> active (inclusive)', () => {
    expect(computeSaleState(START, END, END).status).toBe('active');
  });

  it('60s after end -> ended', () => {
    expect(computeSaleState(START, END, END + MIN).status).toBe('ended');
  });
  ...
  it('degenerate window (endsAt <= startsAt) resolves deterministically', () => {
    ...
    expect(computeSaleState(START, START, START).status).toBe('active');
    expect(computeSaleState(START, START, START - 1).status).toBe('upcoming');
    expect(computeSaleState(START, START, START + 1).status).toBe('ended');
    // Inverted window: now < start dominates -> upcoming; now >= start
    // implies now > end -> ended. No active span exists.
    expect(computeSaleState(END, START, START).status).toBe('upcoming');
    expect(computeSaleState(END, START, END).status).toBe('ended');
  });
  ...
  it('server-time skew immunity: gate uses server now, not client now', () => {
    ...
    expect(computeSaleState(START, END, serverNow).status).toBe('active');
    ...
    expect(computeSaleState(START, END, skewedFast).status).toBe('ended');
    expect(computeSaleState(START, END, skewedSlow).status).toBe('upcoming');
  });
});
